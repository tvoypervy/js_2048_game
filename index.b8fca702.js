...
function updateBoard() {
    ...
    cells.forEach((cell, index)=>{
        ...
        const col = index % 4;
        if (!boardState[row] || boardState[row][col] === undefined) return;
...
function updateStatus() {
    ...
    hideAllMessages();
    if (st === "win") messageWin.classList.remove("hidden");
    else if (st === "lose") messageLose.classList.remove("hidden");
    else if (st === "idle") messageStart.classList.remove("hidden");
}
function updateUI() {
    updateBoard();
    updateScore();
...
function handleStartClick() {
    if (game.getStatus() === "playing" || game.isStarted) game.restart();
    else game.start();
    updateUI();
}
...
function init() {
    if (!startButton || !gameField || !scoreElement) return;
...
class Game {
    ...
    constructor(initialState){
        ...
        this.board = initialState ? this.deepCopy(initialState) : this.deepCopy(EMPTY_BOARD);
    ...
    processLine(line) {
        ...
        for(let i = 0; i < filtredLine.length - 1; i++)if (filtredLine[i] === filtredLine[i + 1]) {
            filtredLine[i] *= 2;
            scoreChange += filtredLine[i];
            filtredLine[i + 1] = 0;
        }
        filtredLine = filtredLine.filter((value)=>value !== 0);
        while(filtredLine.length < 4)filtredLine.push(0);
        return {
            newLine: filtredLine,
            scoreChange
        };
    ...
    transposeBoard(board) {
        ...
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++)newBoard[r][c] = board[c][r];
        return newBoard;
    }
    checkWin() {
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
            ...
            if (c < 3 && this.board[r][c + 1] === current) return true;
            if (r < 3 && this.board[r + 1][c] === current) return true;
        }
        return false;
    }
    hasAvailableMoves() {
    ...
    moveLeft() {
        ...
        for(let row = 0; row < 4; row++){
            ...
            if (oldBoard[row].join(",") !== newLine.join(",")) boardChange = true;
    ...
    moveRight() {
        ...
        for(let row = 0; row < 4; row++){
            ...
            this.score += scoreChange;
            newLine.reverse();
            if (oldBoard[row].join(",") !== newLine.join(",")) boardChanged = true;
            this.board[row] = newLine;
        }
        if (boardChanged) {
            this.generateNewTile();
    ...
    moveUp() {
        ...
        this.board = this.transposeBoard(this.board);
        for(let r = 0; r < 4; r++)if (oldBoard[r].join(",") !== this.board[r].join(",")) {
            boardChanged = true;
            break;
        }
        if (boardChanged) {
